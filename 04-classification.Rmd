...
Since we are using a K-nearest neighbor model, it is important that the variables are centered and scaled to make sure that the variables have a uniform influence. We can accomplish this transformation with `step_normalize()`, which does centering and scaling in one go.
...
The variable of interest, *number of bike rentals per hour* can take on non-negative integer values. This makes Poisson Regression a suitable candidate to model the data.
